 ...
 *
 * <p>1. 1-byte booleans (Type.boolType()), 2. 4-byte integers (Type.intType()), 3. 4-byte floats
 * (Type.floatType()), and 4. n-byte strings (Type.stringType(n)) where n > 0. 5. 8-byte integers
 * (Type.longType())
 *
 * <p>Note that n-byte strings and m-byte strings are considered different types when n != m.
 */
public class Type {
