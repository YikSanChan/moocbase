...
/**
 * A DataBox is an element of one of the primitive types specified in Type.java. You can create
 *
 * <p>- booleans with new BoolDataBox(b), - integers with new IntDataBox(i), - floats with new
 * FloatDataBox(f), - strings with new StringDataBox(s, n), and - longs with new LongDataBox(l).
 *
 * <p>You can unwrap a databox by first pattern matching on its type and then using one of getBool,
 * getInt, getFloat, getString, and getLong:
 *
 * <p>Databox d = DataBox.fromBytes(bytes); switch (d.type().getTypeId()) { case BOOL: {
 * System.out.println(d.getBool()); } case INT: { System.out.println(d.getInt()); } case FLOAT: {
 * System.out.println(d.getFloat()); } case STRING: { System.out.println(d.getString()); } case
 * LONG: { System.out.println(d.getLong()); } }
 */
