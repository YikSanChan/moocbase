...
/**
 * This class represents the full name of a resource. The name of a resource is an ordered tuple of
 * integers, and any subsequence of the tuple starting with the first element is the name of a
 * resource higher up on the hierarchy. For debugging aid, we attach a string to each integer (which
 * is only used in toString()).
 *
 * <p>For example, a page may have the name (0, 3, 10), where 3 is the table's partition number and
 * 10 is the page number. We store this as the list [("database, 0), ("Students", 3), ("10", 10)],
 * and its ancestors on the hierarchy would be [("database", 0)] (which represents the entire
 * database), and [("database", 0), ("Students", 3)] (which represents the Students table, of which
 * this is a page of).
 */
public class ResourceName {
  ...
  private ResourceName(List<Pair<String, Long>> names) {
    ...
    this.hash =
        names.stream().map(x -> x == null ? null : x.getSecond()).collect(toList()).hashCode();
  }
