...
class InnerNode extends BPlusNode {
  ...
  /**
   * Returns the largest number d such that the serialization of an InnerNode with 2d keys will fit
   * on a single page.
   */
   ...
   *
   * <p>numLessThanEqual(0, Arrays.asList(1, 2, 3, 4, 5)) == 0 numLessThanEqual(1, Arrays.asList(1,
   ...
   * numLessThanEqual(5, Arrays.asList(1, 2, 3, 4, 5)) == 5 numLessThanEqual(6, Arrays.asList(1, 2,
   * 3, 4, 5)) == 5
   *
   * <p>This helper function is useful when we're navigating down a B+ tree and need to decide which
   * child to visit. For example, imagine an index node with the following 4 keys and 5 children
   * pointers:
   *
   * <p>+---+---+---+---+ | a | b | c | d | +---+---+---+---+ / | | | \ 0 1 2 3 4
   *
   * <p>If we're searching the tree for value c, then we need to visit child 3. Not coincidentally,
  ...
  /**
   * An inner node on page 0 with a single key k and two children on page 1 and 2 is turned into the
   * following DOT fragment:
   *
   * <p>node0[label = "<f0>|k|<f1>"]; ... // children "node0":f0 -> "node1"; "node0":f1 -> "node2";
   */
  @Override
  public String toDot() {
  ...
  public static InnerNode fromBytes(
      BPlusTreeMetadata metadata,
      BufferManager bufferManager,
      LockContext treeContext,
      long pageNum) {
