...
/**
 * This transaction context implementation assumes that exactly one transaction runs on a thread at
 * a time, and that, aside from the unblock() method, no methods of the transaction are called from
 * a different thread than the thread that the transaction is associated with. This implementation
 * blocks the thread when block() is called.
 */
