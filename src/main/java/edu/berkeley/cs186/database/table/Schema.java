...
/**
 * The schema of a table includes the name and type of every one of its fields. For example, the
 * following schema:
 *
 * <p>List<String> fieldNames = Arrays.asList("x", "y"); List<Type> fieldTypes =
 * Arrays.asList(Type.intType(), Type.floatType()); Schema s = new Schema(fieldNames, fieldSize);
 *
 * <p>represents a table with an int field named "x" and a float field named "y".
 */
public class Schema {
  ...
  Record verify(List<DataBox> values) {
    if (values.size() != fieldNames.size()) {
      String err =
          String.format("Expected %d values, but got %d.", fieldNames.size(), values.size());
