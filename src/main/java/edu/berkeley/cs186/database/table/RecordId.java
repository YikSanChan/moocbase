...
/**
 * A Record in a particular table is uniquely identified by its page number (the number of the page
 * on which it resides) and its entry number (the record's index in the page). A RecordId is a pair
 * of the page number and entry number.
 */
public class RecordId implements Comparable<RecordId> {
  ...
  public byte[] toBytes() {
    ...
    // 2-byte short.
    return ByteBuffer.allocate(getSizeInBytes()).putLong(pageNum).putShort(entryNum).array();
  }
