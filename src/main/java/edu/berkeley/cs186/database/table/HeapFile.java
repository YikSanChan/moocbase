...
/**
 * Interface for a heap file, which receives requests for pages with a certain amount of space, and
 * returns a page with enough space. Assumes a packed page layout.
 */
public interface HeapFile extends BacktrackingIterable<Page> {
  ...
  /**
   * Fetches a data page with a certain amount of unused space. New data and header pages may be
   * allocated as necessary.
   *
   * @param requiredSpace amount of space needed; cannot be larger than effectivePageSize -
   *     emptyPageMetadataSize
   * @return pinned page with the requested area of contiguous space
   */
  Page getPageWithSpace(short requiredSpace);

  /**
   * Updates the amount of free space on a page. Updating to effectivePageSize frees the page, and
