...
/**
 * # Overview A Table represents a database table with which users can insert, get, update, and
 * delete records:
 *
 * <p>// Create a brand new table t(x: int, y: int) which is persisted in the // the heap file
 * heapFile. List<String> fieldNames = Arrays.asList("x", "y"); List<String> fieldTypes =
 * Arrays.asList(Type.intType(), Type.intType()); Schema schema = new Schema(fieldNames,
 * fieldTypes); Table t = new Table("t", schema, heapFile, new DummyLockContext());
 *
 * <p>// Insert, get, update, and delete records. List<DataBox> a = Arrays.asList(new IntDataBox(1),
 * new IntDataBox(2)); List<DataBox> b = Arrays.asList(new IntDataBox(3), new IntDataBox(4));
 * RecordId rid = t.addRecord(a); Record ra = t.getRecord(rid); t.updateRecord(b, rid); Record rb =
 * t.getRecord(rid); t.deleteRecord(rid);
 *
 * <p># Persistence Every table is persisted in its own HeapFile object (passed into the
 * constructor), which interfaces with the BufferManager and DiskSpaceManager to save it to disk.
 *
 * <p>A table can be loaded again by simply constructing it with the same parameters.
 *
 * <p># Storage Format Now, we discuss how tables serialize their data.
 *
 * <p>All pages are data pages - there are no header pages, because all metadata is stored elsewhere
 * (as rows in the information_schema.tables table). Every data page begins with a n-byte bitmap
 * followed by m records. The bitmap indicates which records in the page are valid. The values of n
 * and m are set to maximize the number of records per page (see computeDataPageNumbers for
 * details).
 *
 * <p>For example, here is a cartoon of what a table's file would look like if we had 5-byte pages
 * and 1-byte records:
 *
 * <p>+----------+----------+----------+----------+----------+ \ Page 0 | 1001xxxx | 01111010 |
 * xxxxxxxx | xxxxxxxx | 01100001 | | +----------+----------+----------+----------+----------+ |
 * Page 1 | 1101xxxx | 01110010 | 01100100 | xxxxxxxx | 01101111 | |- data
 * +----------+----------+----------+----------+----------+ | Page 2 | 0011xxxx | xxxxxxxx |
 * xxxxxxxx | 01111010 | 00100001 | | +----------+----------+----------+----------+----------+ /
 * \________/ \________/ \________/ \________/ \________/ bitmap record 0 record 1 record 2 record 3
 *
 * <p>- The first page (Page 0) is a data page. The first byte of this data page is a bitmap, and
 * the next four bytes are each records. The first and fourth bit are set indicating that record 0
 * and record 3 are valid. Record 1 and record 2 are invalid, so we ignore their contents.
 * Similarly, the last four bits of the bitmap are unused, so we ignore their contents. - The second
 * and third page (Page 1 and 2) are also data pages and are formatted similar to Page 0.
 *
 * <p>When we add a record to a table, we add it to the very first free slot in the table. See
 * addRecord for more information.
 *
 * <p>Some tables have large records. In order to efficiently handle tables with large records (that
 * still fit on a page), we format these tables a bit differently, by giving each record a full
 * page. Tables with full page records do not have a bitmap. Instead, each allocated page is a
 * single record, and we indicate that a page does not contain a record by simply freeing the page.
 *
 * <p>In some cases, this behavior may be desirable even for small records (our database only
 * supports locking at the page level, so in cases where tuple-level locks are necessary even at the
 * cost of an I/O per tuple, a full page record may be desirable), and may be explicitly toggled on
 * with the setFullPageRecords method.
 */
public class Table implements BacktrackingIterable<Record> {
  ...
  // Constructors ////////////////////////////////////////////////////////////
  /**
   * Load a table named `name` with schema `schema` from `heapFile`. `lockContext` is the lock
   * context of the table (use a DummyLockContext() to disable locking). A new table will be created
   * if none exists on the heapfile.
   */
  public Table(String name, Schema schema, HeapFile heapFile, LockContext lockContext) {
    ...
    this.heapFile.setEmptyPageMetadataSize(
        (short) (heapFile.getEffectivePageSize() - numRecordsPerPage * schema.getSizeInBytes()));
  ...
  public void setFullPageRecords() {
    numRecordsPerPage = 1;
    bitmapSizeInBytes = 0;
    heapFile.setEmptyPageMetadataSize(
        (short) (heapFile.getEffectivePageSize() - schema.getSizeInBytes()));
  }
  ...
  // Modifiers ///////////////////////////////////////////////////////////////
  /**
   * buildStatistics builds histograms on each of the columns of a table. Running it multiple times
  ...
  /**
   * addRecord adds a record to this table and returns the record id of the newly added record.
   * stats, freePageNums, and numRecords are updated accordingly. The record is added to the first
   * free slot of the first free page (if one exists, otherwise one is allocated). For example, if
   * the first free page has bitmap 0b11101000, then the record is inserted into the page with index
   * 3 and the bitmap is updated to 0b11111000.
   */
  ...
  /** Retrieves a record from the table, throwing an exception if no such record exists. */
  ...
  /**
   * Overwrites an existing record with new values and returns the existing record. stats is updated
   * accordingly. An exception is thrown if rid does not correspond to an existing record in the
   * table.
   */
  ...
  /**
   * Deletes and returns the record specified by rid from the table and updates stats, freePageNums,
   * and numRecords as necessary. An exception is thrown if rid does not correspond to an existing
   * record in the table.
   */
  public synchronized Record deleteRecord(RecordId rid) {
    ...
    try {
      ...
      heapFile.updateFreeSpace(
          page, (short) ((numRecordsPerPage - numRecords) * schema.getSizeInBytes()));
      this.numRecords--;
  ...
  /**
   * Recall that every data page contains an m-byte bitmap followed by n records. The following
   * three functions computes m and n such that n is maximized. To simplify things, we round n down
   * to the nearest multiple of 8 if necessary. m and n are stored in bitmapSizeInBytes and
   * numRecordsPerPage respectively.
   *
   * <p>Some examples:
   *
   * <p>| Page Size | Record Size | bitmapSizeInBytes | numRecordsPerPage | | --------- |
   * ----------- | ----------------- | ----------------- | | 9 bytes | 1 byte | 1 | 8 | | 10 bytes |
   * 1 byte | 1 | 8 | ... | 17 bytes | 1 byte | 1 | 8 | | 18 bytes | 2 byte | 2 | 16 | | 19 bytes |
   * 2 byte | 2 | 16 |
   */
  ...
  /**
   * Enables auto-escalation. All future requests for pages of this table by transactions that hold
   * locks on at least 20% of the locks on the table's pages when this table has at least 10 pages
   * should escalate to a table-level lock before any locks are requested.
   */
  ...
  /**
   * Disables auto-escalation. No future requests for pages of this table should result in an
   * automatic escalation to a table-level lock.
   */
  ...
  private BacktrackingIterator<RecordId> blockRidIterator(Iterator<Page> pageIter, int maxPages) {
    ...
    return new ConcatBacktrackingIterator<>(
        new PageIterator(new ArrayBacktrackingIterator<>(block), true));
  }

  public BacktrackingIterator<Record> blockIterator(Iterator<Page> block, int maxPages) {
  ...
  /**
   * RIDPageIterator is a BacktrackingIterator over the RecordIds of a single page of the table.
   *
   * <p>See comments on the BacktrackingIterator interface for how mark and reset should function.
   */
