 ...
 * record lengths do not need to be fixed-length).
 *
 * <p>Header pages are layed out as follows: - first byte: 0x1 to indicate valid allocated page -
 * next 4 bytes: page directory id - next 8 bytes: page number of next header page, or -1
 * (0xFFFFFFFFFFFFFFFF) if no next header page. - next 10 bytes: page number of data page (or -1),
 * followed by 2 bytes of amount of free space - repeat 10 byte entries
 *
 * <p>Data pages contain a small header containing: - 4-byte page directory id - 4-byte index of
 * which header page manages it - 2-byte offset indicating which slot in the header page its data
 * page entry resides
 *
 * <p>This header is used to quickly locate and update the header page when the amount of free space
 * on the data page changes, as well as ensure that we do not modify pages in other page directories
 * by accident.
 *
 * <p>The page directory id is a randomly generated 32-bit integer used to help detect bugs (where
 * we attempt to write to a page that is not managed by the page directory).
 */
public class PageDirectory implements HeapFile {
  ...
  private static final short HEADER_ENTRY_COUNT =
      (BufferManager.EFFECTIVE_PAGE_SIZE - HEADER_HEADER_SIZE) / DataPageEntry.SIZE;

  // size of the header in data pages
  private static final short DATA_HEADER_SIZE = 10;

  // effective page size
  private static final short EFFECTIVE_PAGE_SIZE =
      BufferManager.EFFECTIVE_PAGE_SIZE - DATA_HEADER_SIZE;
  ...
  /**
   * Creates a new heap file, or loads existing file if one already exists at partNum.
   *
   * @param bufferManager buffer manager
   * @param partNum partition to allocate new header pages in (can be different partition from data
   *     pages)
  ...
  public Page getPageWithSpace(short requiredSpace) {
    ...
    if (requiredSpace > EFFECTIVE_PAGE_SIZE - emptyPageMetadataSize) {
      throw new IllegalArgumentException(
          "requesting page with more space than the size of the page");
    }
  ...
  /**
   * Wrapper around page object to skip the header and verify that it belongs to this page
   * directory.
   */
  ...
  private class HeaderPage implements BacktrackingIterable<Page> {
    ...
    private HeaderPage(long pageNum, int headerOffset, boolean firstHeader) {
      ...
      // This does mean that we do not have complete isolation in the header pages, but this does
      // not
      // really matter, as the only observable effect is that a transaction may be told to use a
      // different
    ...
    private class HeaderPageIterator extends IndexBacktrackingIterator<Page> {
      ...
      protected Page getValue(int index) {
        ...
        try {
          ...
          DataPageEntry dpe = DataPageEntry.fromBytes(b);
          return new DataPage(
              pageDirectoryId, bufferManager.fetchPage(lockContext, dpe.pageNum, false));
        } finally {
          HeaderPage.this.page.unpin();
        }
      }
