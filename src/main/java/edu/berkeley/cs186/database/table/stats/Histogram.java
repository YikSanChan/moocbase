 ...
 * explicitly storing the values. For example, given the following set of numbers:
 *
 * <p>4, 9, 10, 10, 10, 13, 15, 16, 18, 18, 22, 23, 25, 26, 24, 42, 42, 42
 *
 * <p>We can build the following histogram:
 *
 * <p>10 | 9 | 8 8 | +----+ 7 | | | 6 | | | 5 | | | 4 4 | | +----+ 3 3 | 2 | | | +----+ 2 | +----+ |
 * | 1 | | 1 | | | | +----+ | 0 | | | | | | | ------------------------------ 0 10 20 30 40 50
 *
 * <p>A histogram is an ordered list of B "buckets", each of which defines a range (low, high). For
 * the first, B - 1 buckets, the low of the range is inclusive an the high of the range is
 * exclusive. For the last Bucket the high of the range is inclusive as well. Each bucket counts the
 * number of values that fall within its range. In this project, you will work with a floating point
 * histogram where low and high are defined by floats. For any other data type, we will map it so it
 * fits into a floating point histogram.
 *
 * <p>The primary data structure to consider is Bucket<Float>[] buckets, which is a list of Bucket
 * objects
 *
 * <p>Bucket<Float> b = new Bucket(10.0, 100.0); //defines a bucket whose low value is 10 and high
 * is 100 b.getStart(); //returns 10.0 b.getEnd(); //returns 100.0 b.increment(15);// adds the value
 * 15 to the bucket b.getCount();//returns the number of items added to the bucket
...
public class Histogram {
  ...
  /**
   * We only consider float histograms, and these two methods turn every data type into a float. We
   * call this mapping quantization. That means given any DataBox, we turn it into a float number.
   * For Booleans, Integers, Floats, order is preserved in the mapping. But for strings, only
  ...
  /**
   * buildHistogram() takes a table and an attribute and builds a fixed width histogram, with the
   * following procedure.
   *
   * <p>1. Take a pass through the full table, and store the min and the max "quantized" value. 2.
   * Calculate the width which is the (max - min)/#buckets 3. Create empty bucket objects and place
   * them in the array. 4. Populate the buckets by incrementing
   *
   * <p>Edge cases: width = 0, put an item only in the last bucket. final bucket is inclusive on the
   * last value.
   */
  ...
  /**
   * Given a quantized value, set the bucket that contains the value to 1/distinctCount, and set all
   * other values to 0.
   */
  ...
  /**
   * Given a quantized value, set the bucket that contains the value by 1-1/distinctCount, and set
  ...
  /**
   * Given a quantized value, set the bucket that contains the value by (end - q)/width, and set all
   * other buckets to 1 if higher and 0 if lower.
   */
  ...
  /**
   * Given a quantized value, set the bucket that contains the value by (q-start)/width, and set all
   * other buckets to 1 if lower and 0 if higher.
   */
  ...
  /**
   * Return an estimate of the reduction factor for a given filter. For example, consider again the
   * example histogram from the top of the file. The reduction factor for the predicate `>= 25` is
   * 0.5 because roughly half of the values are greater than or equal to 25.
   */
  ...
  /**
   * Given a histogram for a dataset, return a new histogram for the same dataset with a filter
   * applied. For example, if apply the filter `>= 20` to the example histogram from the top of the
   * file, we would get the following histogram:
   *
   * <p>6 | 5 | 4 4 | +----+ 3 3 | | | +----+ 2 | | | 1 | | 1 | 0 0 | +----+ | 0 | +----+----+ | | |
   * ------------------------------ 0 10 20 30 40 50]
   */
