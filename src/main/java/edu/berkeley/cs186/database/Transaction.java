...
public interface Transaction extends AutoCloseable {
  ...
  /**
   * Sets status of transaction. Should not be used directly by users of the transaction (this
   * should be called by the recovery manager).
   *
   ...
   *
   * <p>This is the default way a transaction ends.
   */
  void commit();
   ...
   *
   * <p>Project 5 (Recovery) must be fully implemented.
   */
  void rollback();

  /**
   * Cleanup transaction (when transaction ends). Does not need to be called directly, as
   * commit/rollback should call cleanup themselves. Does not do anything on successive calls when
  ...
  /**
   * Creates a table. Equivalent to CREATE TABLE tableName (...s)
   *
   * <p>Indices must be created afterwards with createIndex.
   *
  ...
  /**
   * Drops a table. Equivalent to DROP TABLE tableName
   *
  ...
  /**
   * Creates an index. Equivalent to CREATE INDEX tableName_columnName ON tableName (columnName) in
   * postgres.
   *
   * <p>The only index supported is a B+ tree. Indices require Project 2 (B+ trees) to be fully
   * implemented. Bulk loading requires Project 3 Part 1 (Joins/Sorting) to be fully implemented as
   * well.
   *
  ...
  /**
   * Drops an index. Equivalent to DROP INDEX tableName_columnName in postgres.
   *
  ...
  /**
   * Returns a QueryPlan selecting from tableName. Equivalent to SELECT * FROM tableName and used
  ...
  /**
   * Returns a QueryPlan selecting from tableName. Equivalent to SELECT * FROM tableName AS alias
  ...
  /**
   * Inserts a row into a table. Equivalent to INSERT INTO tableName VALUES(...values)
   *
  ...
  /**
   * Updates rows in a table. Equivalent to UPDATE tableName SET targetColumnName =
   * targetValue(targetColumnName)
   *
  ...
  /**
   * Updates rows in a table. Equivalent to UPDATE tableName SET targetColumnName =
   * targetValue(targetColumnName) WHERE predColumnName predOperator predValue
   *
   * @param tableName name of table to update
   * @param targetColumnName column to update
   * @param targetValue function mapping old values to new values
  ...
  /**
   * Deletes rows from a table. Equivalent to DELETE FROM tableNAME WHERE predColumnName
  ...
  /**
   * Creates a savepoint. A transaction may roll back to a savepoint it created at any point before
   * committing/aborting. Equivalent to SAVEPOINT savepointName
   *
   * <p>Savepoints require Project 5 (recovery) to be fully implemented.
   *
  ...
  /**
   * Rolls back all changes made by the transaction since the given savepoint. Equivalent to
  ...
  /**
   * Deletes a savepoint. Equivalent to RELEASE SAVEPOINT
   *
   * <p>Savepoints require Project 5 (recovery) to be fully implemented.
   *
   * @param savepointName name of savepoint
