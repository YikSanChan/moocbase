...
public class EndCheckpointLogRecord extends LogRecord {
  ...
  public byte[] toBytes() {
    int recordSize =
        getRecordSize(
            dirtyPageTable.size(), transactionTable.size(), touchedPages.size(), numTouchedPages);
    ...
    for (Map.Entry<Long, List<Long>> entry : touchedPages.entrySet()) {
      buf.putLong(entry.getKey()).putShort((short) entry.getValue().size());
  ...
  public static int getRecordSize(
      int numDPTRecords, int numTxnTableRecords, int touchedPagesMapSize, int numTouchedPages) {
    // DPT: long -> long (16 bytes)
    // xact: long -> (byte, long) (17 bytes)
    // touched pages: long -> (short (size) + longs) (10 bytes + 8 bytes per page)
    return 7
        + 16 * numDPTRecords
        + 17 * numTxnTableRecords
        + 10 * touchedPagesMapSize
        + 8 * numTouchedPages;
  }

  /**
   * @return boolean indicating whether information for the log record can fit in one record on a
   *     page
   */
  public static boolean fitsInOneRecord(
      int numDPTRecords, int numTxnTableRecords, int touchedPagesMapSize, int numTouchedPages) {
    int recordSize =
        getRecordSize(numDPTRecords, numTxnTableRecords, touchedPagesMapSize, numTouchedPages);
