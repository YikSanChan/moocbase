...
/**
 * The LogManager is responsible for interfacing with the log itself. The log is stored on its own
 * partition (partition 0). Since log pages are never deleted, the page number is always increasing,
 * so we assign LSNs as follow: - page 1: [ LSN 10000, LSN 10040, LSN 10080, ...] - page 2: [ LSN
 * 20000, LSN 20030, LSN 20055, ...] - page 3: [ LSN 30000, LSN 30047, LSN 30090, ...] allowing for
 * up to 10,000 log entries per page. The index (last 4 digits) is the offset within the page where
 * the log record starts. Log entries are not fixed width, so backwards iteration is not as easy as
 * forward iteration. Page 0 is reserved for the master record, which only contains a few log
 * entries: the master record, with LSN 0, followed by an empty begin and end checkpoint record. The
 * master record is the only record in the entire log that may be rewritten.
 *
 * <p>The LogManager also is responsible for writing pageLSNs onto pages and flushing the log when
 * pages are flushed, and therefore has a few methods that must be called by the buffer manager when
 * pages are fetched and evicted (fetchPageHook, fetchNewPageHook, and pageEvictHook). These must be
 * called from the buffer manager to ensure that pageLSN is up to date, and that flushedLSN >= any
 * pageLSN on disk.
 */
class LogManagerImpl implements LogManager {
  ...
  public synchronized long appendToLog(LogRecord record) {
    ...
    do {
      if (logTailBuffer == null
          || bytes.length > DiskSpaceManager.PAGE_SIZE - logTailBuffer.position()) {
        logTailPinned = true;
  ...
  /**
   * Flushes the log to at least the specified record, essentially flushing up to and including the
   * page that contains the record specified by the LSN.
   *
