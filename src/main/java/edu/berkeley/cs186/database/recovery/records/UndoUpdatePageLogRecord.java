...
public class UndoUpdatePageLogRecord extends LogRecord {
  ...
  public UndoUpdatePageLogRecord(
      long transNum, long pageNum, long prevLSN, long undoNextLSN, short offset, byte[] after) {
  ...
  public byte[] toBytes() {
    byte[] b =
        new byte[(after.length == BufferManager.EFFECTIVE_PAGE_SIZE ? 36 : 37) + after.length];
    Buffer buf =
        ByteBuffer.wrap(b)
  ...
  public static Optional<LogRecord> fromBytes(Buffer buf) {
    ...
    buf.get(after);
    return Optional.of(
        new UndoUpdatePageLogRecord(transNum, pageNum, prevLSN, undoNextLSN, offset, after));
  }
