...
public abstract class LogRecord {
  ...
  public final long getLSN() {
    if (LSN == null) {
      throw new IllegalStateException(
          "LSN not set, has this log record been through a log manager call yet?");
    }
    return LSN;
  }
  ...
  /** @return optional instance containing page number of data that is changed by transaction */
  public Optional<Long> getPageNum() {
    return Optional.empty();
  }

  /**
   * @return optional instance containing partition number of data that is changed by transaction
   */
  ...
  /**
   * Gets the table of transaction numbers mapped to page numbers of pages that were touched by the
  ...
  /** @return boolean indicating whether transaction recorded in the log record is undoable */
  public boolean isUndoable() {
    return false;
  }

  /** @return boolean indicating whether transaction recorded in the log record is redoable */
   ...
   * @return the CLR corresponding to this log record, and a boolean that is true if the log must be
   *     flushed up to the CLR after executing the undo, and false otherwise.
   */
   ...
   *
   * <p>- a 1-byte integer indicating the type of log record, followed by - a variable number of
   * bytes depending on log record (see specific LogRecord implementations for details).
   */
  ...
  /**
   * Set the method called whenever redo() is called on a LogRecord. This is only to be used for
   * testing.
   *
