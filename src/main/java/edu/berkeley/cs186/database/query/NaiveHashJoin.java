...
public class NaiveHashJoin {
  ...
  /**
   * This class represents a naive hash join on two relations with records contained in
   * leftRelationIterator and rightRelationIterator. To join the two relations the class will
   * attempt to partition the left records and then probe with the right records. It will fail if
   * any of the partitions are larger than the B-2 pages of memory needed to construct the in memory
   * hash table by throwing an InvalidArgumentException.
   */
  public NaiveHashJoin(
      Iterator<Record> leftRelationIterator,
      BacktrackingIterator<Record> rightRelationIterator,
      int leftColumnIndex,
  ...
  /**
   * Partition stage. For every record in the left record iterator, hashes the value we are joining
   * on and adds that record to the correct partition.
   */
  private void partition(NaiveHashPartition[] partitions, Iterator<Record> leftRecords) {
    while (leftRecords.hasNext()) {
      ...
      if (partitionNum < 0) partitionNum += partitions.length; // Hash might be negative
      partitions[partitionNum].addRecord(record);
    }
  }

  /**
   * Builds the hash table using leftRecords and probes it with the records in rightRecords. Joins
   * the matching records and returns them as the joinedRecords list.
   *
  ...
  private List<Record> buildAndProbe(NaiveHashPartition partition, Iterator<Record> rightRecords) {
    ...
    // Our hash table to build on. The list contains all the records in the left records that hash
    // to the same key
  ...
  /**
   * Runs the Hash Join algorithm! First, run the partitioning stage to create an array of Hash
   * Partitions Then, build and probe with each hash partitions records Finally, return our list of
   * joined records.
   *
  ...
  /**
   * Create an appropriate number of NaiveHashPartitions and return them as an array.
   *
  ...
  private NaiveHashPartition[] createPartitions() {
    ...
    for (int i = 0; i < usableBuffers; i++) partitions[i] = createPartition();
    return partitions;
  }
