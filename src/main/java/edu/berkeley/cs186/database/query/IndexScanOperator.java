...
class IndexScanOperator extends QueryOperator {
  ...
  public TableStats estimateStats() {
    ...
    return stats.copyWithPredicate(this.columnIndex, this.predicate, this.value);
  }
  ...
  public int estimateIOCost() {
    ...
    int count =
        tableStats.getHistograms().get(columnIndex).copyWithPredicate(predicate, value).getCount();
  ...
  private class IndexScanIterator implements Iterator<Record> {
    ...
    private IndexScanIterator() {
      ...
      if (IndexScanOperator.this.predicate == PredicateOperator.EQUALS) {
        this.sourceIterator =
            IndexScanOperator.this.transaction.lookupKey(
        ...
        this.sourceIterator =
            IndexScanOperator.this.transaction.sortedScan(
                IndexScanOperator.this.tableName, IndexScanOperator.this.columnName);
      } else if (IndexScanOperator.this.predicate == PredicateOperator.GREATER_THAN) {
        this.sourceIterator =
            IndexScanOperator.this.transaction.sortedScanFrom(
                IndexScanOperator.this.tableName,
                IndexScanOperator.this.columnName,
                IndexScanOperator.this.value);
        while (this.sourceIterator.hasNext()) {
          ...
          if (r.getValues()
                  .get(IndexScanOperator.this.columnIndex)
                  .compareTo(IndexScanOperator.this.value)
              > 0) {
            this.nextRecord = r;
            break;
          }
