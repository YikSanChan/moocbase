...
abstract class JoinOperator extends QueryOperator {
  ...
  /**
   * Create a join operator that pulls tuples from leftSource and rightSource. Returns tuples for
   * which leftColumnName and rightColumnName are equal.
   *
  ...
  public QueryOperator getSource() {
    throw new QueryPlanException(
        "There is no single source for join operators. Please use "
  ...
  public Schema computeSchema() {
    ...
    if (!leftSchemaTypes
        .get(this.leftColumnIndex)
        .getClass()
        .equals(rightSchemaTypes.get(this.rightColumnIndex).getClass())) {
      throw new QueryPlanException(
          "Mismatched types of columns " + leftColumnName + " and " + rightColumnName + ".");
    }
  ...
  public TableStats estimateStats() {
    ...
    return leftStats.copyWithJoin(this.leftColumnIndex, rightStats, this.rightColumnIndex);
  }
  ...
  /**
   * All iterators for subclasses of JoinOperator should subclass from JoinIterator; JoinIterator
   * handles creating temporary tables out of the left and right input operators.
   */
  protected abstract class JoinIterator implements Iterator<Record> {
    ...
    public JoinIterator() {
      if (JoinOperator.this.getLeftSource().isSequentialScan()) {
        this.leftTableName =
            ((SequentialScanOperator) JoinOperator.this.getLeftSource()).getTableName();
      } else {
        this.leftTableName =
            JoinOperator.this.createTempTable(JoinOperator.this.getLeftSource().getOutputSchema());
      ...
      if (JoinOperator.this.getRightSource().isSequentialScan()) {
        this.rightTableName =
            ((SequentialScanOperator) JoinOperator.this.getRightSource()).getTableName();
      } else {
        this.rightTableName =
            JoinOperator.this.createTempTable(JoinOperator.this.getRightSource().getOutputSchema());
