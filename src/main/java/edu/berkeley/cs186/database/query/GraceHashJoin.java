...
public class GraceHashJoin {
  ...
  /** Main constructor */
  public GraceHashJoin(
      Iterator<Record> leftRelationIterator,
      Iterator<Record> rightRelationIterator,
      int leftColumnIndex,
   ...
   * Partition stage.
   *
   * <p>For every record in the given iterator, hashes the value from the column we're joining on
   * and adds it to the correct partition in partitions.
   *
   * <p>It will be helpful to take a look at HashPartition.java to see what methods you can use to
   * add the record to the correct partition.
   */
  private void partition(
      HashPartition[] partitions, Iterator<Record> records, boolean left, int pass) {
    assert pass >= 1;
  ...
  /**
   * Runs the buildAndProbe stage on a given partition. To make things easier we set up 5 variables
   * for you to use for the building and probing phases.
   *
   * <p>Returns a list of the joined records
   *
   ...
   * otherwise we should apply the grace hash join algorithm recursively to break up the partitions
   * further.
   *
   * <p>Once all partitions have been conquered return a list of all the joined records from
   * leftRecords and rightRecords
   *
  ...
  private List<Record> run(Iterator<Record> leftRecords, Iterator<Record> rightRecords, int pass) {
    ...
    for (HashPartition partition : partitions) {
      ...
      // It may be helpful to read through the HashPartition.java class for methods that will be
      // useful here
    }
    return joinedRecords;
  }
  ...
  /**
   * Create an appropriate number of HashPartitions relative to the number of available buffers we
   * have and return an array
   *
   * <p>You should first know the answer to how many partitions will be created!
   *
  ...
  // Provided Helpers ////////////////////////////////////////////////////////
  /**
   * Helper method to create a joined record from a record of the left relation and a record of the
   * right relation.
   *
  ...
  /**
   * Helper method to join together a record from the build stage to a record from the probe stage.
   * Since build or probe could be from either the left or right partition, make sure you set
   * probeFirst properly!
   *
   ...
   *
   * <p>Come up with two lists of records for leftRecords and rightRecords such that NHJ will error
   * when given those relations, but GHJ will successfully run.
   *
   * <p>createRecord() takes in a value and returns a record with that value.
   *
   * <p>Lists cannot be empty or the joins will error!
   *
   * <p>You may find the following information useful: Maximum 976 Records per page B = 6
   *
   ...
   *
   * <p>Come up with two lists of records for leftRecords and rightRecords such that GHJ will error
   * (in our case hit the max pass cap).
   *
   * <p>createRecord() takes in a value and returns a record with that value.
   *
   * <p>Lists cannot be empty or the joins will error!
