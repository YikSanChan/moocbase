...
/**
 * QueryPlan provides a set of functions to generate simple queries. Calling the methods
 * corresponding to SQL syntax stores the information in the QueryPlan, and calling execute
 * generates and executes a QueryPlan DAG.
 */
public class QueryPlan {
  ...
  /**
   * Creates a new QueryPlan within transaction. The base table is startTableName, aliased to
   * aliasTableName.
   *
   * @param transaction the transaction containing this query
   * @param startTableName the source table for this query
  ...
  /**
   * Join the leftColumnName column of the existing queryplan against the rightColumnName column of
   * tableName.
   *
  ...
  /**
   * Join the leftColumnName column of the existing queryplan against the rightColumnName column of
   * tableName, aliased as aliasTableName.
   *
   * @param tableName the table to join against
   ...
   */
  public void join(
      String tableName, String aliasTableName, String leftColumnName, String rightColumnName) {
  ...
  /**
   * Generates a naive QueryPlan in which all joins are at the bottom of the DAG followed by all
   * select predicates, an optional group by operator, and a set of projects (in that order).
   *
  ...
  /**
   * Gets all SELECT predicates for which there exists an index on the column referenced in that
   * predicate for the given table.
   *
  ...
  /**
   * Applies all eligible SELECT predicates to a given source, except for the predicate at index
   * except. The purpose of except is because there might be one SELECT predicate that was already
   * used for an index scan, so no point applying it again. A SELECT predicate is represented as
   * elements of this.selectColumnNames, this.selectOperators, and this.selectDataBoxes that
  ...
  private QueryOperator addEligibleSelections(QueryOperator source, int except) {
    for (int i = 0; i < this.selectColumnNames.size(); i++) {
      ...
      try {
        String colName =
            source.checkSchemaForColumn(source.getOutputSchema(), selectColumnNames.get(i));
  ...
  /**
   * Finds the lowest cost QueryOperator that scans the given table. First determine the cost of a
   * sequential scan for the given table. Then for every index that can be used on that table,
   * determine the cost of an index scan. Keep track of the minimum cost operation. Then push down
   * eligible projects (SELECT predicates). If an index scan was chosen, exclude that SELECT
   * predicate when pushing down selects. This method will be called during the first pass of the
   * search algorithm to determine the most efficient way to access each single table.
   *
   * @return a QueryOperator that has the lowest cost of scanning the given table which is either a
   *     SequentialScanOperator or an IndexScanOperator nested within any possible pushed down
   *     select operators
   */
  ...
  /**
   * Given a join condition between an outer relation represented by leftOp and an inner relation
   * represented by rightOp, find the lowest cost join operator out of all the possible join types
   * in JoinOperator.JoinType.
   *
   * @return lowest cost join QueryOperator between the input operators
   */
  private QueryOperator minCostJoinType(
  ...
  /**
   * Iterate through all table sets in the previous pass of the search. For each table set, check
   * each join predicate to see if there is a valid join condition with a new table. If so, check
   * the cost of each type of join and keep the minimum cost join. Construct and return a mapping of
   * each set of table names being joined to its lowest cost join operator. A join predicate is
   * represented as elements of this.joinTableNames, this.joinLeftColumnNames, and
   * this.joinRightColumnNames that correspond to the same index of these lists.
   *
   * @return a mapping of table names to a join QueryOperator
   */
  Map<Set<String>, QueryOperator> minCostJoins(
      Map<Set<String>, QueryOperator> prevMap, Map<Set<String>, QueryOperator> pass1Map) {
    ...
    // Input: pass1Map (each set is a singleton with one table and single table access query
    // operator)
  ...
  /**
   * Finds the lowest cost QueryOperator in the given mapping. A mapping is generated on each pass
   * of the search algorithm, and relates a set of tables to the lowest cost QueryOperator accessing
   * those tables. This method is called at the end of the search algorithm after all passes have
   * been processed.
   *
  ...
  private void generateIndexPlan(String indexColumn) {
    ...
    this.finalOperator =
        new IndexScanOperator(this.transaction, this.startTableName, indexColumn, operator, value);
    ...
    for (String joinTable : this.joinTableNames) {
      ...
      this.finalOperator =
          new SNLJOperator(
              finalOperator,
              scanOperator,
              this.joinLeftColumnNames.get(index),
              this.joinRightColumnNames.get(index),
              this.transaction);
  ...
  private void addGroupBy() {
    if (this.groupByColumn != null) {
      ...
      this.finalOperator =
          new GroupByOperator(this.finalOperator, this.transaction, this.groupByColumn);
    }
  }
