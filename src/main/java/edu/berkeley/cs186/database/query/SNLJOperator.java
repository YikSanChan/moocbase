...
public class SNLJOperator extends JoinOperator {
  public SNLJOperator(
    ...
    super(leftSource, rightSource, leftColumnName, rightColumnName, transaction, JoinType.SNLJ);
  ...
  /**
   * An implementation of Iterator that provides an iterator interface for this operator. Note that
   * the left table is the "outer" loop and the right table is the "inner" loop.
   */
  private class SNLJIterator extends JoinIterator {
     ...
     * There is always a first record. If there were no first records (empty rightSource) then the
     * code would not have made it this far. See line 66.
     */
     ...
     *
     * <p>The thrown exception means we're done: there is no next record It causes
     * this.fetchNextRecord (the caller) to hand control to its caller.
     */
    ...
    /**
     * Pre-fetches what will be the next record, and puts it in this.nextRecord. Pre-fetching
     * simplifies the logic of this.hasNext() and this.next()
     */
      ...
      do {
        if (this.rightRecord != null) {
          ...
          DataBox leftJoinValue =
              this.leftRecord.getValues().get(SNLJOperator.this.getLeftColumnIndex());
          DataBox rightJoinValue =
              rightRecord.getValues().get(SNLJOperator.this.getRightColumnIndex());
