...
/**
 * Database objects keeps track of transactions, tables, and indices and delegates work to its disk
 * manager, buffer manager, lock manager and recovery manager.
 *
 * <p>A Database instance operates on files in the directory specified by the `fileDir` argument in
 * the constructors. The files in this directory will be modified by a DiskSpaceManager instance.
 * Upon starting up for the following partitions are allocated through the disk space manager: -
 * Partition 0: used for log records from the recovery manager - Partition 1: used by the
 * information_schema.tables table, which persists information about user created tables - Partition
 * 2: used by the information_schema.indices table, which persists information about user created
 * indices
 *
 * <p>Each partition corresponds to a file in `fileDir`. The remaining partitions are used for user
 * created tables and are allocated as tables are created. Internally user created tables are stored
 * with the prefix `tables.` to distinguish them from the metadata tables prefixed with
 * `information_schema.` For example, if a user creates a table `Students` then internally it will
...
public class Database implements AutoCloseable {
  ...
  /**
   * Creates a new database with: - Default buffer size - Locking disabled (DummyLockManager) -
   * Clock eviction policy - Recovery manager disabled (DummyRecoverManager)
   *
  ...
  /**
   * Creates a new database with defaults: - Locking disabled (DummyLockManager) - Clock eviction
   * policy - Recovery manager disabled (DummyRecoverManager)
   *
   * @param fileDir the directory to put the table files in
  ...
  /**
   * Creates a new database with defaults: - Clock eviction policy - Recovery manager disabled
   * (DummyRecoverManager)
   *
   * @param fileDir the directory to put the table files in
   * @param numMemoryPages the number of pages of memory in the buffer cache
   ...
   */
  public Database(
      String fileDir, int numMemoryPages, LockManager lockManager, EvictionPolicy policy) {
  ...
  public Database(
    ...
    if (useRecoveryManager) {
      recoveryManager =
          new ARIESRecoveryManager(
              lockManager.databaseContext(),
              this::beginRecoveryTransaction,
    ...
    bufferManager =
        new BufferManagerImpl(diskSpaceManager, recoveryManager, numMemoryPages, policy);
  ...
  private void initTableInfo() {
    ...
    LockContext tableInfoContext = getTableInfoContext();
    HeapFile tableInfoHeapFile =
        new PageDirectory(bufferManager, 1, tableInfoPage0, (short) 0, tableInfoContext);
    tableInfo =
        new Table(TABLE_INFO_TABLE_NAME, getTableInfoSchema(), tableInfoHeapFile, tableInfoContext);
  ...
  private void initIndexInfo() {
    ...
    HeapFile heapFile =
        new PageDirectory(bufferManager, 2, indexInfoPage0, (short) 0, indexInfoContext);
  ...
  private void loadMetadataTables() {
    ...
    HeapFile tableInfoHeapFile =
        new PageDirectory(
            bufferManager,
            1,
            DiskSpaceManager.getVirtualPageNum(1, 0),
            (short) 0,
            tableInfoContext);
    tableInfo =
        new Table(TABLE_INFO_TABLE_NAME, getTableInfoSchema(), tableInfoHeapFile, tableInfoContext);
    ...
    LockContext indexInfoContext = getIndexInfoContext();
    HeapFile indexInfoHeapFile =
        new PageDirectory(
            bufferManager,
            2,
            DiskSpaceManager.getVirtualPageNum(2, 0),
            (short) 0,
            indexInfoContext);
    indexInfo =
        new Table(INDEX_INFO_TABLE_NAME, getIndexInfoSchema(), indexInfoHeapFile, indexInfoContext);
    indexInfo.disableAutoEscalate();
    indexInfo.setFullPageRecords();
    tableLookup.put(INDEX_INFO_TABLE_NAME, indexInfo);
    tableIndices.put(INDEX_INFO_TABLE_NAME, Collections.emptyList());
      ...
      try {
        ...
        executor.execute(
            () -> {
              ...
              // X(table) acquired during table ctor; not needed earlier because no one can even
              // check
              // if table exists due to X(table metadata) lock
              LockContext tableContext = getTableContext(record.tableName, record.partNum);
              HeapFile heapFile =
                  new PageDirectory(
                      bufferManager, record.partNum, record.pageNum, (short) 0, tableContext);
              ...
              synchronized (lockManager) {
                LockContext metadataContext =
                    getTableInfoContext().childContext(recordId.getPageNum());
          ...
          () -> {
            ...
            try {
              ...
              if (!tableIndices.containsKey(metadata.getTableName())) {
                // the list only needs to be synchronized while indices are being loaded, as
                // multiple
                // indices may attempt to add themselves to the list at the same time
                tableIndices.put(
                    metadata.getTableName(), Collections.synchronizedList(new ArrayList<>()));
              }
  ...
  /**
   * @return Schema for information_schema.tables with fields: | field name | field type
   *     --+--------------+------------------------- 0 | table_name | string(32) 1 | part_num | int
   *     2 | page_num | long 3 | is_temporary | bool 4 | schema | string(MAX_SCHEMA_SIZE)
   */
  ...
  /**
   * @return Schema for information_schema.indices with fields: | field name | field type
   *     --+---------------------+------------ 0 | table_name | string(32) 1 | col_name | string(32)
   *     2 | order | int 3 | part_num | int 4 | root_page_num | long 5 | key_schema_typeid | int 6 |
   *     key_schema_typesize | int 7 | height | int
   */
  ...
  private BPlusTreeMetadata parseIndexMetadata(Record record) {
    ...
    return new BPlusTreeMetadata(
        tableName, colName, keySchema, order, partNum, rootPageNum, height);
  }
  ...
  void lockTableMetadata(String tableName, LockType lockType) {
    ...
    if (mayNeedToCreate) {
      // TODO(proj4_part3): acquire all locks needed on database/information_schema.tables before
      // compute()
      tableInfoLookup.compute(
          tableName,
          (tableName_, recordId) -> {
  ...
  void lockIndexMetadata(String indexName, LockType lockType) {
    ...
    if (mayNeedToCreate) {
      // TODO(proj4_part3): acquire all locks needed on database/information_schema.indices before
      // compute()
      indexInfoLookup.compute(
          indexName,
          (indexName_, recordId) -> {
            if (recordId != null) { // record created between containsKey call and this
              return recordId;
            ...
            return Database.this.indexInfo.addRecord(
                Arrays.asList(
  ...
  /**
   * Gets the transaction number counter. This is the number of transactions that have been created
   * so far, and also the number of the next transaction to be created.
   */
  ...
  private class TransactionContextImpl extends AbstractTransactionContext {
    ...
    public String createTempTable(Schema schema) {
      ...
      HeapFile heapFile =
          new PageDirectory(bufferManager, partNum, pageNum, (short) 0, lockContext);
      ...
      try {
        ...
        try {
          return new SortOperator(
                  this, tableName, Comparator.comparing((Record r) -> r.getValues().get(offset)))
              .iterator();
    ...
    @Override
    public Iterator<Record> sortedScanFrom(
        String tableName, String columnName, DataBox startValue) {
      // TODO(proj4_part3): scan locking
    ...
    @Override
    public BacktrackingIterator<Record> getBlockIterator(
    ...
    public RecordId addRecord(String tableName, List<DataBox> values) {
      ...
      for (String indexName : tableIndices.get(tab.getName())) {
        ...
        resolveIndexFromName(tableName, column)
            .getSecond()
            .put(values.get(colNames.indexOf(column)), rid);
      }
      return rid;
    }
    ...
    public RecordId deleteRecord(String tableName, RecordId rid) {
      ...
      for (String indexName : tableIndices.get(tab.getName())) {
        ...
        resolveIndexFromName(tableName, column)
            .getSecond()
            .remove(values.get(colNames.indexOf(column)));
      }
      return rid;
    }
    ...
    @Override
    public void runUpdateRecordWhere(
        String tableName,
    ...
    @Override
    public void runDeleteRecordWhere(
        String tableName,
        String predColumnName,
        PredicateOperator predOperator,
        DataBox predValue) {
    ...
    private Pair<String, BPlusTreeMetadata> resolveIndexMetadataFromName(
    ...
    private Pair<String, BPlusTree> resolveIndexFromName(String tableName, String columnName) {
  ...
  private class TransactionImpl extends AbstractTransaction {
    ...
    public void createTable(Schema s, String tableName) {
      ...
      try {
        ...
        HeapFile heapFile =
            new PageDirectory(
                bufferManager, record.partNum, record.pageNum, (short) 0, tableContext);
        tableLookup.put(prefixedTableName, new Table(prefixedTableName, s, heapFile, tableContext));
      ...
      try {
        ...
        if (!schemaColNames.contains(columnName)) {
          throw new DatabaseException(
              "table " + tableName + " does not have a column " + columnName);
        }
    ...
    @Override
    public void update(
        String tableName, String targetColumnName, UnaryOperator<DataBox> targetValue) {
    ...
    public void update(
      ...
      TransactionContext.setTransaction(transactionContext);
      try {
        transactionContext.runUpdateRecordWhere(
            tableName, targetColumnName, targetValue, predColumnName, predOperator, predValue);
      } finally {
        TransactionContext.unsetTransaction();
      }
    }
