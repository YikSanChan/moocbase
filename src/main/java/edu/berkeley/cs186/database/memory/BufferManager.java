...
public interface BufferManager extends AutoCloseable {
  ...
  /**
   * Frees a page - evicts the page from cache, and tells the disk space manager that the page is no
   * longer needed. Page must be pinned before this call, and cannot be used after this call (aside
   * from unpinning).
   *
  ...
  /**
   * Frees a partition - evicts all relevant pages from cache, and tells the disk space manager that
   * the partition is no longer needed. No pages in the partition may be pinned before this call,
  ...
  /**
   * Fetches a buffer frame with data for the specified page. Reuses existing buffer frame if page
   * already loaded in memory. Pins the buffer frame. Cannot be used outside the package.
   *
   * @param pageNum page number
   * @param logPage whether the page is for the log or not
  ...
  /**
   * Fetches a buffer frame for a new page. Pins the buffer frame. Cannot be used outside the
   * package.
   *
   * @param partNum partition number for new page
   * @param logPage whether the page is for the log or not
  ...
  /**
   * Calls flush on the frame of a page and unloads the page from the frame. If the page is not
   ...
   *
   * @param process method to consume page numbers. The first parameter is the page number, and the
   *     second parameter is a boolean indicating whether the page is dirty (has an unflushed
   *     change).
   */
