...
/**
 * Implementation of a buffer manager, with configurable page replacement policies. Data is stored
 * in page-sized byte arrays, and returned in a Frame object specific to the page loaded (evicting
 * and loading a new page into the frame will result in a new Frame object, with the same underlying
 * byte array), with old Frame objects backed by the same byte array marked as invalid.
 */
public class BufferManagerImpl implements BufferManager {
  ...
  /**
   * Buffer frame, containing information about the loaded page, wrapped around the underlying byte
   * array. Free frames use the index field to create a (singly) linked list between free frames.
   */
  class Frame extends BufferFrame {
    ...
    /**
     * Pin buffer frame; cannot be evicted while pinned. A "hit" happens when the buffer frame gets
     * pinned.
     */
    @Override
    public void pin() {
          ...
          for (Pair<Integer, Integer> range : changedRanges) {
            ...
            long pageLSN =
                recoveryManager.logPageWrite(
                    transaction.getTransNum(), pageNum, position, before, after);
    ...
    /**
     * Requests a valid Frame object for the page (if invalid, a new Frame object is returned). Page
     * is pinned on return.
     */
    @Override
    ...
    /**
     * Generates (offset, length) pairs for where buf differs from contents. Merges nearby pairs
     * (where nearby is defined as pairs that have fewer than BufferManager.RESERVED_SPACE bytes of
     * unmodified data between them).
     */
  ...
  public Frame fetchPageFrame(long pageNum, boolean logPage) {
    ...
    try {
      ...
      newFrame =
          this.frames[frameIndex] = new Frame(evictedFrame.contents, frameIndex, pageNum, logPage);
