...
public class NaiveHashPartition {
  ...
  /**
   * A class representing a partition used in naive hashing operations. This partition can only hold
   * records from a single relation, as opposed to HashPartition which can hold records from two
   * different relations.
   *
  ...
  public NaiveHashPartition(TransactionContext transaction, Schema schema) {
    this.tempTableName =
        transaction.createTempTable(schema); // Partitions are backed as a tempTable
