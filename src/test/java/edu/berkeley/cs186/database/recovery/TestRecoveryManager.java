...
public class TestRecoveryManager {
  ...
  public void testSimpleCommit() throws Exception {
    ...
    assertTrue(
        logManager.getFlushedLSN() + " is not greater than or equal to " + LSN1,
        LSN1 <= logManager.getFlushedLSN());
    assertTrue(
        logManager.getFlushedLSN() + " is not less than " + LSN2,
  ...
  public void testEnd() throws Exception {
    ...
    assertEquals(
        LogManagerImpl.maxLSN(LogManagerImpl.getLSNPage(LSNs[6])), logManager.getFlushedLSN());
  ...
  public void testSimpleLogPageWrite() throws Exception {
    ...
    long LSN1 =
        recoveryManager.logPageWrite(
            transaction1.getTransNum(), 10000000002L, pageOffset, before, after);
    ...
    long LSN2 =
        recoveryManager.logPageWrite(
            transaction2.getTransNum(), 10000000003L, pageOffset, before, after);
  ...
  public void testTwoPartLogPageWrite() throws Exception {
    ...
    long secondLSN =
        recoveryManager.logPageWrite(transaction1.getTransNum(), pageNum, (short) 0, before, after);
  ...
  public void testRestartAnalysis() throws Exception {
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000001L, 0L, (short) 0, before, after))); // 0
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000002L, LSNs.get(0), (short) 0, before, after))); // 1
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(3L, 10000000003L, 0L, (short) 0, before, after))); // 2
    ...
    assertEquals(
        new HashSet<>(Collections.singletonList(10000000001L)),
    ...
    assertEquals(
        new HashSet<>(Collections.singletonList(10000000003L)),
    ...
    // FlushedLSN
    assertEquals(
        LogManagerImpl.maxLSN(LogManagerImpl.getLSNPage(LSNs.get(7))), logManager.getFlushedLSN());
  }
  ...
  public void testRestartRedo() throws Exception {
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000002L, LSNs.get(0), (short) 1, before, after))); // 1
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000002L, LSNs.get(1), (short) 1, after, before))); // 2
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(2), (short) 2, before, after))); // 3
  ...
  public void testRestartUndo() throws Exception {
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(1), (short) 2, before, after))); // 2
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000004L, LSNs.get(2), (short) 3, before, after))); // 3
    ...
    entry1.touchedPages =
        new HashSet<>(Arrays.asList(10000000001L, 10000000002L, 10000000003L, 10000000004L));
  ...
  public void testUndoCLR() throws Exception { // Releasing public sp20
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000003L, LSNs.get(1), (short) 0, before, after))); // 2
    LSNs.add(
        logManager.appendToLog(
            logManager.fetchLogRecord(LSNs.get(2)).undo(LSNs.get(2)).getFirst())); // 3
    LSNs.add(logManager.appendToLog(new AbortTransactionLogRecord(1L, LSNs.get(3)))); // 4
    LSNs.add(
        logManager.appendToLog(
            logManager.fetchLogRecord(LSNs.get(1)).undo(LSNs.get(4)).getFirst())); // 5
  ...
  public void testUndoDPTAndFlush() throws Exception { // released public sp20
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(1L, 10000000002L, LSNs.get(1), (short) 0, before, after))); // 2
  ...
  public void testAbortingEnd() throws Exception { // Released public sp20
    ...
    byte[] after =
        new byte[] {(byte) 0xBA, (byte) 0xAD, (byte) 0xF0, (byte) 0x00, (byte) 0x00, (byte) 0x0D};
    ...
    LogRecord updateRecord =
        new UpdatePageLogRecord(t1.getTransNum(), 10000000001L, 0L, (short) 71, before, after);
    ...
    assertEquals(
        Collections.singletonMap(10000000001L, updateCLR.getLSN()),
        getDirtyPageTable(recoveryManager));
  ...
  public void testFlushingRollback() throws Exception { // Released public sp20
    ...
    logManager.flushToLSN(29999L); // force next record to be LSN 30000L
    LogRecord updateRecord3 =
        new UpdatePageLogRecord(
            t1.getTransNum(), 91757270000008529L, 20000L, (short) 3030, before, after);
    ...
    Map<Long, TransactionTableEntry> expectedTxnTable =
        new HashMap<>(getTransactionTable(recoveryManager));
  ...
  public void testRestartCleanup1() throws Exception { // Releasing public sp20
    ...
    getLogManager(recoveryManager)
        .fetchLogRecord(LSN1)
        .redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));
  ...
  public void testAnalysisCheckpoints1() throws Exception { // Releasing public sp20
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(2L, 10000000004L, 0L, (short) 0, before, after))); // 1
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(2L, 10000000006L, LSNs.get(1), (short) 0, before, after))); // 2
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(3L, 10000000007L, 0L, (short) 0, before, after))); // 3
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(2L, 10000000005L, LSNs.get(2), (short) 0, before, after))); // 7
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(2L, 10000000003L, LSNs.get(7), (short) 0, before, after))); // 8
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(2L, 10000000003L, LSNs.get(8), (short) 0, after, before))); // 9
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(
                2L, 10000000006L, LSNs.get(9), (short) 0, after, before))); // 10
    ...
    LSNs.add(
        logManager.appendToLog(
            logManager.fetchLogRecord(LSNs.get(3)).undo(LSNs.get(3)).getFirst())); // 13
    ...
    LSNs.add(
        logManager.appendToLog(
            new UpdatePageLogRecord(5L, 10000000001L, 0L, (short) 0, before, after))); // 16
    LSNs.add(
        logManager.appendToLog(
            new EndCheckpointLogRecord(
    ...
    assertEquals(
        new HashMap<Long, TransactionTableEntry>() {
          {
  ...
  public void testNestedRollback() throws Exception {
    ...
    logManager.flushToLSN(9999L); // force next record to be LSN 10000L
    LogRecord updateRecord1 =
        new UpdatePageLogRecord(t1.getTransNum(), 10000000001L, 0L, (short) 71, before, after);
    logManager.appendToLog(updateRecord1);
    updateRecord1.redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));

    logManager.flushToLSN(19999L); // force next record to be LSN 20000L
    LogRecord updateRecord2 =
        new UpdatePageLogRecord(t2.getTransNum(), 10000000001L, 0L, (short) 33, before, after);
    logManager.appendToLog(updateRecord2);
    updateRecord2.redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));
    ...
    logManager.flushToLSN(29999L); // force next record to be LSN 30000L
    LogRecord updateRecord3 =
        new UpdatePageLogRecord(t2.getTransNum(), 10000000001L, 20000L, (short) 11, before, after);
    logManager.appendToLog(updateRecord3);
    updateRecord3.redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));

    logManager.flushToLSN(39999L); // force next record to be LSN 40000L
    LogRecord updateRecord4 =
        new UpdatePageLogRecord(t1.getTransNum(), 10000000002L, 10000L, (short) 991, before, after);
    logManager.appendToLog(updateRecord4);
    updateRecord4.redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));

    logManager.flushToLSN(49999L); // force next record to be LSN 50000L
    LogRecord updateRecord5 =
        new UndoUpdatePageLogRecord(
            t2.getTransNum(), 10000000001L, 30000L, 20000L, (short) 11, before);
    logManager.appendToLog(updateRecord5);
    updateRecord5.redo(getDiskSpaceManager(recoveryManager), getBufferManager(recoveryManager));
  ...
  /**
   * *********************************************************************** Helpers - these are
   * similar to the ones available in TestARIESStudent *
   * ***********************************************************************
   */

  /**
   * Helper to set up checks for redo. The first call to LogRecord.redo will call the first method
   * in METHODS, the second call to the second method in METHODS, and so on. Call this method before
   * the redo pass, and call finishRedoChecks after the redo pass.
   */
  ...
  /**
   * Helper to finish checks for redo. Call this after the redo pass (or undo pass)- if not enough
   * redo calls were performed, an error is thrown.
   *
   * <p>If setupRedoChecks is used for the redo pass, and this method is not called before the undo
   * pass, and the undo pass calls undo at least once, an error may be incorrectly thrown.
   */
  ...
  protected RecoveryManager loadRecoveryManager(String dir) throws Exception {
    RecoveryManager recoveryManager =
        new ARIESRecoveryManagerNoLocking(
    ...
    BufferManager bufferManager =
        new BufferManagerImpl(diskSpaceManager, recoveryManager, 32, new LRUEvictionPolicy());
  ...
  /**
   * Flushes everything to disk, but does not call RecoveryManager#shutdown. Similar to pulling the
   * plug on the database at a time when no changes are in memory. You can simulate a shutdown where
   * certain changes _are_ in memory, by simply never applying them (i.e. write a log record, but do
   * not make the changes on the buffer manager/disk space manager).
   */
