...
public abstract class TestARIESStudentRunnerBase {
  ...
  public TestRule globalTimeout =
      new DisableOnDebug(Timeout.millis((long) (1000 * TimeoutScaling.factor)));
  ...
  public void setup() throws Exception {
    assertNotNull(
        "TestARIESStudent should have @Category({Proj5Tests.class, StudentTests.class})",
        TestARIESStudent.class.getAnnotation(Category.class));
    assertArrayEquals(
        "TestARIESStudent should have @Category({Proj5Tests.class, StudentTests.class})",
        new Class[] {Proj5Tests.class, StudentTests.class},
        TestARIESStudent.class.getAnnotation(Category.class).value());
    for (Method m : TestARIESStudent.class.getMethods()) {
      assertNull(
          "TestARIESStudent methods should not have @Category", m.getAnnotation(Category.class));
    }
  ...
  private static class AnalysisInstance extends TestInstance {
    private AnalysisInstance(Class<? extends RecoveryManager> recoveryManagerClass)
        throws Exception {
  ...
  private static class IntegrationInstance extends TestInstance {
    private IntegrationInstance(Class<? extends RecoveryManager> recoveryManagerClass)
        throws Exception {
  ...
  private abstract static class TestInstance extends TestARIESStudent {
    ...
    private TestInstance(Class<? extends RecoveryManager> recoveryManagerClass, boolean limited)
        throws Exception {
      this.recoveryManagerClass = recoveryManagerClass;
      this.recoveryManagerConstructor =
          recoveryManagerClass.getDeclaredConstructor(
              LockContext.class, Function.class, Consumer.class, Supplier.class, boolean.class);
    ...
    protected RecoveryManager loadRecoveryManager(String dir) throws Exception {
      RecoveryManager recoveryManager =
          new DelegatedRecoveryManager(recoveryManagerClass, recoveryManagerConstructor);
    ...
    protected BufferManager getBufferManager(RecoveryManager recoveryManager) throws Exception {
      DelegatedRecoveryManager drm =
          limited
              ? ((LimitedRecoveryManager) recoveryManager).inner
    ...
    @Override
    protected DiskSpaceManager getDiskSpaceManager(RecoveryManager recoveryManager)
        throws Exception {
      DelegatedRecoveryManager drm =
          limited
              ? ((LimitedRecoveryManager) recoveryManager).inner
      ...
      return (DiskSpaceManager)
          recoveryManagerClass.getDeclaredField("diskSpaceManager").get(drm.inner);
    }
    ...
    protected Map<Long, Long> getDirtyPageTable(RecoveryManager recoveryManager) throws Exception {
      ...
      return (Map<Long, Long>)
          recoveryManagerClass.getDeclaredField("dirtyPageTable").get(drm.inner);
    }
  ...
  private static class DelegatedRecoveryManager implements RecoveryManager {
    ...
    DelegatedRecoveryManager(
        Class<? extends RecoveryManager> recoveryManagerClass,
        Constructor<? extends RecoveryManager> recoveryManagerConstructor)
        throws Exception {
      this.recoveryManagerClass = recoveryManagerClass;
      this.inner =
          recoveryManagerConstructor.newInstance(
    ...
    public long logFreePage(long transNum, long pageNum) {
      ...
      try {
        Map<Long, TransactionTableEntry> transactionTable =
            (Map<Long, TransactionTableEntry>)
                recoveryManagerClass.getDeclaredField("transactionTable").get(inner);
        Map<Long, Long> dirtyPageTable =
            (Map<Long, Long>) recoveryManagerClass.getDeclaredField("dirtyPageTable").get(inner);
