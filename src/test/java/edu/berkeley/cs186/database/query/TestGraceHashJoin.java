...
public class TestGraceHashJoin {
  ...
  public void testSimpleNHJ() {
    try (Transaction transaction = d.beginTransaction()) {
      ...
      NaiveHashJoin nhj =
          new NaiveHashJoin(leftIter, rightIter, 1, 1, transaction.getTransactionContext(), schema);
  ...
  public void testSimpleGHJ() {
    try (Transaction transaction = d.beginTransaction()) {
      ...
      GraceHashJoin ghj =
          new GraceHashJoin(
              leftIter, rightIter, 1, 1, transaction.getTransactionContext(), schema, schema, true);
  ...
  public void testGHJDifferentSchemas() {
    try (Transaction transaction = d.beginTransaction()) {
      ...
      for (int i = 186; i < 1860; i++) {
        ...
        // Unsure the order of join so we join both ways and check that the outputted join is at
        // least one of them
  ...
  public void testBreakNHJButPassGHJ() {
    try (Transaction transaction = d.beginTransaction()) {
      ...
      NaiveHashJoin nhj =
          new NaiveHashJoin(leftIter, rightIter, 0, 0, transaction.getTransactionContext(), schema);
      ...
      try {
        ...
        assertEquals(
            "Naive Hash failed for the wrong reason!",
  ...
  public void testGHJBreak() {
    try (Transaction transaction = d.beginTransaction()) {
      ...
      try {
        ...
        assertEquals(
            "GHJ Failed for the wrong reason...",
