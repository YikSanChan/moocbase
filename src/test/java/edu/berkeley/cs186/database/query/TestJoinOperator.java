...
public class TestJoinOperator {
  ...
  public TestRule globalTimeout =
      new DisableOnDebug(Timeout.millis((long) (4000 * TimeoutScaling.factor)));
  ...
  private void checkIOs(String message, long minIOs, long maxIOs) {
    ...
    assertTrue(
        IOs + " I/Os not between " + minIOs + " and " + maxIOs + message,
  ...
  private void setSourceOperators(
      TestSourceOperator leftSourceOperator,
  ...
  private void setSourceOperators(
      QueryOperator leftSourceOperator, QueryOperator rightSourceOperator) {
  ...
  public void testSimpleJoinPNLJ() {
    try (Transaction transaction = d.beginTransaction()) {
      setSourceOperators(new TestSourceOperator(), new TestSourceOperator(), transaction);
    ...
    try (Transaction transaction = d.beginTransaction()) {
      ...
      for (int i = 0; i < 400 * 2; i++) {
        transaction
            ...
            .addRecord("leftTable", leftTableRecords.get(i).getValues());
        transaction
            .getTransactionContext()
            .addRecord("rightTable", rightTableRecords.get(i).getValues());
      }
