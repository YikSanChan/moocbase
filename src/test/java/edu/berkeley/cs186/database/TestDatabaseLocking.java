...
public class TestDatabaseLocking {
  ...
  @ClassRule public static TemporaryFolder checkFolder = new TemporaryFolder();
  ...
  private static <T extends Comparable<? super T>> void assertSameItems(
  ...
  private static List<String> prepare(Long transNum, String... expected) {
    return Arrays.stream(expected)
        .map((String log) -> String.format(log, transNum))
  ...
  private static List<String> removeMetadataLogs(List<String> log) {
    ...
    if (log.size() >= 2
        && log.get(0).endsWith("database IS")
        && log.get(1).endsWith("database IX")) {
  ...
  private List<RecordId> createTable(String tableName, int pages) {
    ...
    try (Transaction t1 = beginTransaction()) {
      ...
      int numRecords =
          pages * t1.getTransactionContext().getTable(tableName).getNumRecordsPerPage();
  ...
  private List<RecordId> createTableWithIndices(
      String tableName, int pages, List<String> indexColumns) {
    ...
    try (Transaction t1 = beginTransaction()) {
      ...
      for (int i = 0; i < numRecords; ++i) {
        rids.add(
            t1.getTransactionContext()
                .addRecord(tableName, Arrays.asList(new IntDataBox(i), new IntDataBox(i))));
      }
    } finally {
      this.db.waitAllTransactions();
    }
  ...
  public void testSimpleTransactionCleanup() {
    ...
    try {
      ...
      assertTrue(
          "did not acquire all required locks",
    ...
    assertTrue(
        "did not free all required locks",
  ...
  public void testSortedScanLocking() {
    ...
    try (Transaction t2 = beginTransaction()) {
      Iterator<Record> r =
          t2.getTransactionContext()
              .sortedScanFrom(tableName, "int2", new IntDataBox(rids.size() / 2));
      while (r.hasNext()) {
        r.next();
      }
      List<String> log = removeMetadataLogs(lockManager.log);
  ...
  public void testDropAllTables() {
    ...
    try (Transaction t0 = beginTransaction()) {
      ...
      assertEquals(prepare(t0.getTransNum(), "acquire %s database X"), lockManager.log);
    }
  }
}
