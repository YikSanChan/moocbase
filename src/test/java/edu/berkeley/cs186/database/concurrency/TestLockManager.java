...
public class TestLockManager {
  ...
  public TestRule globalTimeout =
      new DisableOnDebug(Timeout.millis((long) (2000 * TimeoutScaling.factor)));

  /**
   * Given a LockManager lockman, checks if transaction holds a Lock specified by type on the
   * resource specified by name
   */
  static boolean holds(
      LockManager lockman, TransactionContext transaction, ResourceName name, LockType type) {
  ...
  public void testSimpleAcquireRelease() {
    /**
     * Transaction 0 acquires an S lock on table0 Transaction 0 acquires an S lock on table1 and
     * releases its lock on table0
     */
    DeterministicRunner runner = new DeterministicRunner(1);
    runner.run(
        0,
        () -> {
          lockman.acquireAndRelease(
              transactions[0], tables[0], LockType.S, Collections.emptyList());
          lockman.acquireAndRelease(
              transactions[0], tables[1], LockType.S, Collections.singletonList(tables[0]));
        });
  ...
  public void testAcquireReleaseQueue() {
    /**
     * Transaction 0 acquires an X lock on table0 Transaction 1 acquires an X lock on table1
     * Transaction 0 attempts to acquire an X lock on table1 and release its X lock on table0
     */
    ...
    runner.run(
        ...
        () ->
            lockman.acquireAndRelease(
                transactions[0], tables[0], LockType.X, Collections.emptyList()));
    runner.run(
        1,
        () ->
            lockman.acquireAndRelease(
                transactions[1], tables[1], LockType.X, Collections.emptyList()));
    runner.run(
        0,
        () ->
            lockman.acquireAndRelease(
                transactions[0], tables[1], LockType.X, Collections.singletonList(tables[0])));
      ...
      runner.run(
          ...
          () ->
              lockman.acquireAndRelease(
                  transactions[0], tables[0], LockType.X, Collections.emptyList()));
      fail(
          "Attempting to acquire a duplicate lock should throw a "
  ...
  public void testAcquireReleaseNotHeld() {
    ...
    try {
      ...
      runner.run(
          ...
          () ->
              lockman.acquireAndRelease(
                  transactions[0], tables[2], LockType.X, Arrays.asList(tables[0], tables[1])));
      fail(
          "Attempting to release a lock that is not held should throw " + "a NoLockHeldException.");
  ...
  public void testAcquireReleaseUpgrade() {
    /**
     * Transaction 0 acquires an S lock on table0 Transaction 0 acquires an X lock on table0 and
     * releases its S lock
     */
    DeterministicRunner runner = new DeterministicRunner(1);
    runner.run(
        ...
        () -> {
          lockman.acquireAndRelease(
              transactions[0], tables[0], LockType.S, Collections.emptyList());
          lockman.acquireAndRelease(
              transactions[0], tables[0], LockType.X, Collections.singletonList(tables[0]));
        });
  ...
  public void testSimpleAcquireLock() {
    /** Transaction 0 acquires an S lock on table0 Transaction 1 acquires an X lock on table1 */
    DeterministicRunner runner = new DeterministicRunner(2);
  ...
  public void testSimpleReleaseLock() {
    /**
     * Transaction 0 acquires an X lock on dbResource Transaction 0 releases its lock on dbResource
     */
    DeterministicRunner runner = new DeterministicRunner(1);
    runner.run(
        0,
  ...
  public void testSimpleConflict() {
    /**
     * Transaction 0 acquires an X lock on dbResource Transaction 1 attempts to acquire an X lock on
     * dbResource but blocks due to a conflict with Transaction 0's X lock
     *
     * <p>After this: Transaction 0 should have an X lock on dbResource Transaction 1 should have no
     * lock on dbResource Transaction 0 should not be blocked Transaction 1 should be blocked
     * (waiting to acquire an X lock on dbResource)
     */
    DeterministicRunner runner = new DeterministicRunner(2);
    ...
    /**
     * Transaction 0 releases its lock on dbResource Transaction 1 should unblock, and acquire an X
     * lock on dbResource
     *
     * <p>After this: Transaction 0 should have no lock on dbResource Transaction 1 should have an X
     * lock dbResource Both transactions should be unblocked
     */
  ...
  public void testSXS() {
    /**
     * Transaction 0 acquires an S lock on dbResource Transaction 1 attempts to acquire an X lock on
     * dbResource but blocks due to a conflict with Transaction 0's S lock Transaction 2 attempts to
     * acquire an S lock on dbResource but blocks due to existing queue
     *
     * <p>After this: dbResource should have an S lock from Transaction 0 dbResource should have [X
     * (T1), S (T2)] in its queue Transaction 0 should be unblocked Transaction 1 should be blocked
    ...
    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.S, 0L)),
    ...
    /**
     * Transaction 0 releases its S lock on dbResource Transaction 1 should unblock and acquire an X
     * lock on dbResource Transaction 2 will move forward in the queue, but still be blocked
     *
     * <p>After this: dbResource should have an X lock from Transaction 1 dbResource should have [S
     * (T2)] in its queue Transaction 0 should be unblocked Transaction 1 should be unblocked
     * Transaction 2 should be blocked
     */
    runner.run(0, () -> lockman.release(transactions[0], dbResource));
    ...
    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.X, 1L)),
        lockman.getLocks(dbResource));
    ...
    /**
     * Transaction 1 releases its X lock on dbResource Transaction 2 should unblock and acquire an S
     * lock on dbResource
     *
     * <p>After this: dbResource should have an S lock from Transaction 1 All transactions should be
     * unblocked
     */
    ...
    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.S, 2L)),
        lockman.getLocks(dbResource));
  ...
  public void testXSXS() {
    /**
     * Transaction 0 acquires an X lock on dbResource Transaction 1 attempts to acquire an S lock on
     * dbResource but blocks due to a conflict with Transaction 0's X lock Transaction 2 attempts to
     * acquire an X lock on dbResource but blocks due to an existing queue Transaction 3 attempts to
     * acquire an S lock on dbResource but blocks due to an existing queue
     *
     * <p>After this: Transaction 0 should have an X lock on dbResource dbResources should have [S
     * (T1), X (T2), S (T3)] in its queue Transaction 0 should be unblocked, the rest should be
     * blocked
     */
    ...
    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.X, 0L)),
        lockman.getLocks(dbResource));
    ...
    /**
     * Transaction 0 releases its X lock on dbResource Transaction 1 acquires an S lock on
     * dbResource and unblocks Transaction 2 moves forwards in the queue but remains blocked because
     * of a conflict with Transaction 1's S lock Transaction 3 moves forwards in the queue but
     * remains blocked behind Transaction 2
     *
     * <p>After this: Transaction 1 should have an S lock on dbResource dbResources should have [X
     * (T2), S (T3)] in its queue Transaction 0 and 1 should be unblocked, 2 and 3 should be blocked
     */
    runner.run(0, () -> lockman.release(transactions[0], dbResource));

    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.S, 1L)),
        lockman.getLocks(dbResource));
    ...
    /**
     * Transaction 1 releases its S lock on dbResource Transaction 2 acquires an X lock on
     * dbResource and unblocks Transaction 3 moves forward in the queue but remains blocked because
     * of a conflict with Transaction 2's X lock
     *
     * <p>After this: Transaction 2 should have an X lock on dbResource dbResources should have [S
     * (T3)] in its queue Transaction 0, 1 and 2 should be unblocked, 3 should blocked
     */
    runner.run(1, () -> lockman.release(transactions[1], dbResource));

    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.X, 2L)),
        lockman.getLocks(dbResource));
    ...
    /**
     * Transaction 2 releases its X lock on dbResource Transaction 3 acquires an S lock on
     * dbResource and unblocks
     *
     * <p>After this: Transaction 3 should have an S lock on dbResource dbResources should have an
     * empty queue All transactions should be unblocked
     */
    runner.run(2, () -> lockman.release(transactions[2], dbResource));

    // Lock check
    assertEquals(
        Collections.singletonList(new Lock(dbResource, LockType.S, 3L)),
        lockman.getLocks(dbResource));
  ...
  public void testSimplePromoteLock() {
    /**
     * Transaction 0 acquires an S lock on dbResource Transaction 0 promotes its S lock on
     * dbResource to an X lock
     */
    DeterministicRunner runner = new DeterministicRunner(1);
    runner.run(
        0,
  ...
  public void testSimplePromoteLockNotHeld() {
    ...
    try {
      ...
      fail(
          "Attempting to promote a lock that doesn't exist should "
  ...
  public void testSimplePromoteLockAlreadyHeld() {
    ...
    try {
      runner.run(0, () -> lockman.promote(transactions[0], dbResource, LockType.X));
      fail(
          "Attempting to promote a lock to an equivalent lock should "
  ...
  public void testFIFOQueueLocks() {
    /**
     * Transaction 0 acquires an X lock on dbResource Transaction 1 attempts to acquire an X lock on
     * dbResource but blocks because of a conflict with Transaction 0's X lock Transaction 2
     * attempts to acquire an X lock on dbResource but blocks because of an existing queue
     *
     * <p>After this: Transaction 0 should have an X lock on dbResource dbResource should have [X
     * (T1), X (T2)] in its queue
     */
    DeterministicRunner runner = new DeterministicRunner(3);
    runner.run(0, () -> lockman.acquire(transactions[0], dbResource, LockType.X));
    runner.run(1, () -> lockman.acquire(transactions[1], dbResource, LockType.X));
    ...
    /**
     * Transaction 0 releases its X lock on dbResource Transaction 1 acquires an X lock on
     * dbResource and unblocks Transaction 2 moves forwards in the queue but remains blocked because
     * of a conflict with Transaction 1's X lock
     *
     * <p>After this: Transaction 1 should have an X lock on dbResource dbResource should have [X
     * (T2)] in its queue
     */
    runner.run(0, () -> lockman.release(transactions[0], dbResource));
    ...
    /**
     * Transaction 1 releases its X lock on dbResource Transaction 2 acquires an X lock on
     * dbResource and unblocks
     *
     * <p>After this: Transaction 2 should have an X lock on dbResource
     */
    runner.run(1, () -> lockman.release(transactions[1], dbResource));
  ...
  public void testTableEventualUpgrade() {
    ...
    /**
     * Transaction 0 acquires an S lock on dbResource Transaction 1 acquires an S lock on dbResource
     */
    DeterministicRunner runner = new DeterministicRunner(2);
    ...
    /**
     * Transaction 0 attempts to promote its S lock to an X lock but fails due to a conflict with
     * Transaction 1's S lock. Transaction 0 queues an X lock and blocks.
     */
    ...
    /**
     * Transaction 1 releases its S lock on dbResource Transaction 0 promotes its S lock to an X
     * lock and unblocks
     */
  ...
  public void testIntentBlockedAcquire() {
    ...
    /**
     * Transaction 0 acquires an S lock on dbResource Transaction 1 attempts to acquire an IX lock
     * on dbResource, but blocks due to a conflict with Transaction 0's S lock
     */
    runner.run(0, () -> lockman.acquire(t0, dbResource, LockType.S));
    ...
    /**
     * Transaction 0 releases its S lock on dbResource Transaction 1 acquires an IX lock on
     * dbResource and unblocks
     */
    runner.run(0, () -> lockman.release(t0, dbResource));
  ...
  public void testReleaseUnheldLock() {
    ...
    try {
      ...
      fail(
          "Releasing a lock on a resource you don't hold a lock on "
