...
public class TestLockContext {
  ...
  public void testSimpleAcquireFail() {
    ...
    try {
      ...
      fail(
          "Attempting to acquire an X lock with an IS lock on "
  ...
  public void testSimpleAcquirePass() {
    ...
    Assert.assertEquals(
        Arrays.asList(
            new Lock(dbLockContext.getResourceName(), LockType.IS, 0L),
  ...
  public void testTreeAcquirePass() {
    ...
    Assert.assertEquals(
        Arrays.asList(
            new Lock(dbLockContext.getResourceName(), LockType.IX, 0L),
  ...
  public void testSimpleReleasePass() {
    ...
    Assert.assertEquals(
        Collections.singletonList(new Lock(dbLockContext.getResourceName(), LockType.IS, 0L)),
        lockManager.getLocks(transactions[0]));
  }
  ...
  public void testSimpleReleaseFail() {
    ...
    try {
      ...
      fail(
          "Attemptng to release an IS lock when a child resource "
