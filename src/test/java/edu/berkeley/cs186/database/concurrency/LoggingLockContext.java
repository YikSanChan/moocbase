...
public class LoggingLockContext extends LockContext {
  ...
  /**
   * A special LockContext that works with a LoggingLockManager to emit logs when the user uses
   * disableChildLocks() or capacity()
   */
  LoggingLockContext(LoggingLockManager lockman, LockContext parent, Pair<String, Long> name) {
    super(lockman, parent, name);
  }

  private LoggingLockContext(
      LoggingLockManager lockman, LockContext parent, Pair<String, Long> name, boolean readonly) {
    super(lockman, parent, name, readonly);
  }

  /**
   * Disables locking children. This causes all child contexts of this context to be readonly. This
   * is used for indices and temporary tables (where we disallow finer-grain locks), the former due
   * to complexity locking B+ trees, and the latter due to the fact that temporary tables are only
  ...
  public synchronized LockContext childContext(String readable, long name) {
    LockContext temp =
        new LoggingLockContext(
            (LoggingLockManager) lockman,
            this,
            new Pair<>(readable, name),
