...
public class TestBPlusTree {
  ...
  public TestRule globalTimeout =
      new DisableOnDebug(Timeout.millis((long) (3000 * TimeoutScaling.factor)));
  ...
  public void setup() {
    ...
    this.bufferManager =
        new BufferManagerImpl(
            diskSpaceManager, new DummyRecoveryManager(), 1024, new ClockEvictionPolicy());
  ...
  private void setBPlusTreeMetadata(Type keySchema, int order) {
    this.metadata =
        new BPlusTreeMetadata(
            "test", "col", keySchema, order, 0, DiskSpaceManager.INVALID_PAGE_NUM, -1);
  }
  ...
  private <T> List<T> indexIteratorToList(
      Supplier<Iterator<T>> iteratorSupplier, Iterator<Integer> maxIOsOverride) {
    ...
    assertFalse(
        "too many I/Os used constructing iterator ("
            + (newIOs - prevIOs)
            + " > "
            + maxIOs
    ...
    while (iter.hasNext()) {
      ...
      assertFalse(
          "too many I/Os used per next() call ("
              + (newIOs - prevIOs)
              + " > "
              + maxIOs
              + ") - are you materializing more than you need?",
    ...
    for (int d = 2; d < 5; ++d) {
      ...
      for (int n = 0; n < 2; ++n) {
        ...
        for (int i = 0; i < keys.size(); i += 100) {
          ...
          assertEquals(
              expected, indexIteratorToList(() -> tree.scanGreaterEqual(new IntDataBox(j))));
        }
